...
SDCurve.prototype.init = function(params){
	...
	this._catmullAlpha = params.hasOwnProperty("catmullAlpha") ? params.catmullAlpha : 0.5;
	
	this.subdivide();
};

SDCurve.prototype.isAcceptableType = function(type) {
	var s = ["bspline","interpolating","bezier","catmull-rom"];
...
SDCurve.prototype.catmullAlpha = function(newalpha) {
	if(newalpha != null && newalpha != this._catmullAlpha) {
		this._catmullAlpha = Math.min(Math.max(0, newalpha), 1);
		this.subdivide();
	}
	return this._catmullAlpha;
}
...
SDCurve.prototype.subdivide = function(){
	...
	if(this._fine.length > 2) {
		if(this._type == "interpolating") {
			...
			function getMirror(P0, P1) {
				return P0;
				//return _this.applyWeights([{weight:2, fine: P0}, {weight: -1, fine: P1} ]);
			}
			
			var newfine = [getMirror(this._fine[0], this._fine[1])];
			Array.prototype.push.apply(newfine,this._fine);
			newfine.push(getMirror(this._fine[this._fine.length-1], this._fine[this._fine.length-2]));
			
			this._fine = newfine;
			newfine = [];
			
			for(var crindex=0; crindex<this._fine.length-3; crindex++) {
				var P0 = this._fine[crindex], P1 = this._fine[crindex+1],P2 = this._fine[crindex+2],P3 = this._fine[crindex+3];
				
				function tj(ti, Pi, Pj) {
					return Math.pow(Math.sqrt(Math.pow((Pj.point.x - Pi.point.x),2) + Math.pow((Pj.point.y - Pi.point.y),2)),_this._catmullAlpha) + ti;
				}
				...
				var npts = this._resolution*this._resolution + 1;
				
				for(var i=0; i<npts; i++) {
					var t = t1 + (t2-t1) * i / (npts-1);
					var A1 = t1-t0==0 ? P0 : _this.applyWeights([{weight:(t1-t)/(t1-t0), fine: P0}, {weight: (t-t0)/(t1-t0), fine: P1} ]);
					var A2 = t2-t1==0 ? P1 : _this.applyWeights([{weight:(t2-t)/(t2-t1), fine: P1}, {weight: (t-t1)/(t2-t1), fine: P2} ]);
					var A3 = t3-t2==0 ? P2 : _this.applyWeights([{weight:(t3-t)/(t3-t2), fine: P2}, {weight: (t-t2)/(t3-t2), fine: P3} ]);
					
					var B1 = t2-t0==0 ? A1 : _this.applyWeights([{weight:(t2-t)/(t2-t0), fine: A1}, {weight: (t-t0)/(t2-t0), fine: A2} ]);
					var B2 = t3-t1==0 ? A2 : _this.applyWeights([{weight:(t3-t)/(t3-t1), fine: A2}, {weight: (t-t1)/(t3-t1), fine: A3} ]);
					
					var C = t2-t1==0 ? B1 : _this.applyWeights([{weight:(t2-t)/(t2-t1), fine: B1}, {weight: (t-t1)/(t2-t1), fine: B2} ]);
					newfine.push(C);
				}
			}
